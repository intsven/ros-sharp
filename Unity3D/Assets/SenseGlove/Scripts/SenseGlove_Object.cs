...
public class SenseGlove_Object : MonoBehaviour
{
    //--------------------------------------------------------------------------------------------------------------------------
    // Properties
    public static SenseGlove_Object instance;

    public SenseGloveCs.GloveData rawData;
    ...
    protected void UpdateGlove()
    {
        if (this.wasConnected && this.linkedGlove != null && this.IsConnected) //if we are connected then linkedGlove is assumed not null.
        {
            ...
            //job.Schedule();
            if(!gotlinked && UpdateThread.linkedGlove == null)
            {
                UpdateThread.linkedGlove = linkedGlove;
                gotlinked = true;
            }
            
            if(UpdateThread.rawData != null)
            {
                this.linkedGloveData.UpdateVariables(UpdateThread.rawData);
                //Debug.Log(UpdateThread.rawData.imuValues[0]);
            }
    ...
    {
        public static SenseGlove linkedGlove;
        Solver solver;
        bool limitFingers;
        bool updateWrist;

        public static SenseGloveCs.GloveData rawData;
        public static bool running;

        public UpdateThread(Solver solver, bool limitFingers, bool updateWrist)
        {
            this.solver = solver;
            this.limitFingers = limitFingers;
            this.updateWrist = updateWrist;
            running = true;
        }
